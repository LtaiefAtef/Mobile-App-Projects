...
const getLocation = async () => {
  ...
  const { reportDataRef, setUser1Progress } = useAccidentReport();
  const { sessionData, updateBackendSession, setSessionData, inSession } = useSharedAccidentReport();
  const saveAndRedirect = async () => {
    reportDataRef.current = {
        ...reportDataRef.current,
        accidentDate: form.accidentDate.toDateString(),
        submittedAt: new Date().toDateString(),
        accidentLocation: form.address,
        injuries: form.injuries,
        witnesses: form.witnesses,
        otherVehiclesDamaged: {
          otherVehicleInvolved: form.otherVehiclesDamaged.otherVehicleInvolved,
          numberOfVehicles: Number(form.otherVehiclesDamaged.numberOfVehicles),
        },
      };
    ...
    router.push({ pathname: "/(accident_report)/step-3" });
  };
  return (
    <KeyboardAvoidingView
