...
const pollStatus = (job_id: string) => {
  setTimeout(async () => {
    try {
      ...
      const { status, text } = await xhrRequest('GET', `${API_URL}/status/${job_id}`);
      console.log('[4] Poll status:', status, text);
      ...
      if (status === 200) {
        console.log('[5] Done! Loading image...');
        const viewUrl = `${API_URL}/view/${job_id}?t=${Date.now()}`;
        console.log('[5b] View URL:', viewUrl);
          setResultImage(viewUrl);
          setLoading(false);
        return;
      }
...
const handleGenerate = () => {
  ...
  (async () => {
    try {
      ...
      const { status, text } = await xhrRequest('POST', `${API_URL}/generate`, JSON.stringify({ prompt }));
      console.log('[2] Response:', status, text);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (status !== 'granted') { Alert.alert('Permission needed', 'Storage access is required to save.'); return; }
      const FS = require('expo-file-system');
      const fileUri = FS.cacheDirectory + `generated_${Date.now()}.png`;
      const downloadResult = await FS.downloadAsync(resultImage, fileUri);
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, marginTop: 120 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView style={styles.screen} contentContainerStyle={styles.screenContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {loading && (
          <View style={styles.card}>
            <View style={styles.loadingBox}>
              ...
              <Text style={styles.loadingSubtitle}>This usually takes 15–60 seconds depending on your hardware</Text>
            </View>
          </View>
        )}
        ...
        {resultImage && !loading && (
          <View style={styles.card}>
            <Text style={styles.sectionTitle}>Your diagram</Text>
