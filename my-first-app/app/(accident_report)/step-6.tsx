...
export default function Step6(): React.JSX.Element {
...
//  --- Saving Signature and redirecting to next step ---
    const { reportDataRef, selectedDriver, switchDriver, setUser1Progress, setUser2Progress } = useAccidentReport();
    ...
    const saveAndRedirect = async() => {
        ...
        if(inSession.current && sessionData){
          ...
          if(isAuthor){
            reportDataRef.current = { ...reportDataRef.current, signatures: { ...reportDataRef.current.signatures,
            vehicleA: { signed:isSigned, signedAt:new Date().toDateString(), svgData } } };
            updateBackendSession({ ...sessionData.sharedData, user1Progress:7, sender:sessionData?.createdBy, action:"progress" })
            setSessionData((prev : SessionData) => ({ ...prev, sharedData:{ ...prev.sharedData, user1Progress:7 } })
            )
            router.push("/(accident_report)/step-7");
          }else{
            const user = await getUser();
            reportDataRef.current = {...reportDataRef.current, signatures: { ...reportDataRef.current.signatures, 
            vehicleB: { signed:isSigned, signedAt:new Date().toDateString(), svgData } } };
            sessionDataRef.current = {...sessionDataRef.current, sharedData:{...sessionDataRef.current.sharedData, user2Progress:8 }}
            ...
            setSessionData((prev : SessionData) => ({ ...prev, sharedData:{ ...prev.sharedData, user2Progress:8 } }))
        ...
        if(selectedDriver === "driverA"){
            ...
            reportDataRef.current = {...reportDataRef.current, signatures: {...reportDataRef.current.signatures ,vehicleA: { signed:isSigned, signedAt:new Date().toDateString(), svgData } } };
            switchDriver();
            router.push("/(accident_report)/step-3");
        }else{
            if(!inSession.current){
              setUser2Progress(7);
            }
